...
**Màu** — theme `Docs/socom-beauty-theme.json`, nhưng **merge vào bản copy của `assets/base.json`** của skill trước khi đăng ký (giữ safeguard: padding textbox, cardVisual zero-padding, table grow-to-fit, ẩn visual header). Không đăng ký theme tối giản trần.
...
**Nav trái:** 4 nút (Tổng quan · Hủy đơn · Giỏ hàng · MBA) — bookmark + button. Footer mọi trang: *"Nguồn: SOCOM · dữ liệu synthetic 2021 · số minh hoạ phương pháp"*.
...
## Trạng thái model (đã xong qua MCP — xem `Docs/PROJECT_BRIEF.md` §5)

| Hạng mục | Trạng thái |
| --- | --- |
| `_Measures` (22 measure / 6 folder) | ✅ |
...
1. ✅ (qua MCP) Mark Date table · 22 measure · calc column · `_MBA_Pairs_Matrix`.
...
3. **Theme** — merge màu `socom-beauty-theme.json` vào bản copy `assets/base.json`, đăng ký vào PBIP.
...
Region ghi theo `[col_start, row_start, col_end_excl, row_end_excl]` trên lưới 12×12 (cột 1..13, hàng 1..13). Model đã có sẵn — xem `PROJECT_BRIEF.md` §5.
...
Design Brief:
  ...
  theme_base: assets/base.json  # merge socom-beauty-theme.json dataColors/tokens INTO a copy; keep base safeguards
